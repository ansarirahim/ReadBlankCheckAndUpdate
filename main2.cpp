...
//printf("\n%.2X %.2X %.2X, Adress=%ld", addresshigh,addressmid,addresslow,Spi_address );
			char readcommand[4] = {
				READ,
				addresshigh,
				addressmid,
...
{
printf("(HEX DUMP)=>%.2X %.2X %.2X %.3X\n",addresshigh,addressmid,fromIndex,toIndex);
			  for (ret = fromIndex; ret <(fromIndex+toIndex); ret++) {         //prints returned array on the screen
			      if (!(ret % 16))
			         puts("");
			      printf("%.2X", rx[ret]);
			   }
