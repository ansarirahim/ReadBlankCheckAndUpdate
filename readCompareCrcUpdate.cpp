...

{
	int ret = 0;
	int fd;
csNumber=atoi(argv[1]);
switch(csNumber)
{
	case 0 :
	system("RELAY_OPERATION  35840");
	break;
	case 1:
	system("RELAY_OPERATION 19456");
	break;
	case 2:
	system("RELAY_OPERATION 3080");
	break;
	case 3:
	system("RELAY_OPERATION 3076");
...
sprintf(fileName,"/root/BBB_INTEL_HEX_FORMAT_SPI/CRC41.CS%d",csNumber);
...
//char fileName[100];
 memset(fileName,'\0',100);
        sprintf(fileName,"/root/BBB_INTEL_HEX_FORMAT_SPI/RESULT.CS%d",csNumber);
            fstream f2(fileName, f2.out | f2.trunc);
            f2 << resultStatus << endl;
            f2.close();
